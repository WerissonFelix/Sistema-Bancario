...
def animacao(txt='saindo', animar = False, vari = 0):
    print(txt)
    if (animar):
        for i in range(0,3):
...
def entrar():
    if len(pessoas) == 0:
        print('\033[31;41m Ainda não há nenhuma conta para entrar \033[m')
        sleep(2)
        iniciar()
    else:    
...
def menu_principal():
    ...
    if c == '1' : # menu do inserir
        ...
        limpar()
        traco()
        exclui = escolha("SN","EXCLUIR CONTA? (sim/não) ")
        if exclui[0] == "S" :
            ...
            for i in range(0,3):
                ...
                print('.',end='',flush=True)
            sleep(1)  
            desativados.append(pessoas[posi]) 
            print('Tchau!')
            iniciar()
        elif exclui == "N" :
            menu_principal()  
...
def pix(): # 1° opção do menu inserir
    ...
    if len(pessoas) == 1:
        print('Ainda não há ninguém para mandar pix')
        sleep(2)
        menu_principal()
    else:    
        destinatario = escolha('1234567890','Digite a chave pix do destinatário (cpf) ', tamanho=11)    
        for desa in desativados:
            for valor in desa:
                if valor == destinatario:
                    print('Atenção! o destinatário que você degitou foi excluido!')
                    sleep(3)
                    inserir()
        for i in pessoas:
            for valor in i:
        ...
        if valor_pix > 0 :      
            ...
            if pessoas[posi][2] >= valor_pix :
                if enviar[0] == "S":
                    ...
                    print(f"PIX NO VALOR DE {valor_pix} R$ FOI ENVIADO PARA {destinatario} POR {pessoas[posi][1]} NO HORÁRIO:  {datetime.now().hour} h  ")
